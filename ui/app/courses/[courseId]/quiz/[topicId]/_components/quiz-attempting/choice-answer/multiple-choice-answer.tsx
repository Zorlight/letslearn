...
const answerVariants = cva(
  "flex flex-row items-center gap-3 py-2 px-3 rounded-md cursor-pointer transition-all duration-200 border",
  {
    variants: {
      variant: {
        default: "bg-indigo-100 hover:bg-blue-200 text-slate-600", // when not select and not show correct answer
        ...
        missing: "bg-white text-slate-600 border-red-500", // when not select and show correct answer and this is correct answer
        notSelected: "bg-white text-slate-600", // when not select and show correct answer and this is incorrect answer
      },
    },
    defaultVariants: {
      variant: "default",
...
const MultipleChoiceAnswer = ({
  ...
  return (
    ...
    >
      ...
      {state === "missing" && <span className="text-red-500">✗</span>}
